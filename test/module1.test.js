...
describe("Conway's Game of Life", () => {
  const trimOfWhitespace = (s) =>
    typeof s == "undefined" ? "" : s.replace(/\s/g, "");
  ...
  describe("Finding living neighbors", () => {
    it("Should have a getLivingNeighbors function. @getLivingNeighbors-function", () => {
      ...
      const rPentominoNeighborsOfFourThree =
        gameoflife.getLivingNeighbors([4, 3], rPentomino) || [];

      assert.equal(
        rPentominoNeighborsOfFourThree.length,
        4,
      ...
      assert.equal(
        gameoflife.getLivingNeighbors([6, 6], rPentomino).length,
        0,
        "Have you created a function 'getLivingNeighbors' that returns the living neighbors of the given cell?"
      );
    });
  ...
  describe("Calculating a cell's future state", () => {
    it("Should have a willBeAlive function. @willBeAlive-function", () => {
      ...
      const nextGen = cells.filter((c) =>
        gameoflife.willBeAlive(c, rPentomino)
      );
      assert.equal(
        nextGen.length,
        6,
  ...
  describe("Calculating the next state", () => {
    it("Should have a calculateNext function. @calculateNext-function", () => {
      ...
      const next = gameoflife.calculateNext(
        gameoflife.seed([3, 2], [2, 3], [3, 3], [3, 4], [4, 4])
      );
  ...
  describe("Calculating multiple game states", () => {
    it("Should have an iterate function. @iterate-function", () => {
      ...
      const states =
        gameoflife.iterate(gameoflife.startPatterns.square, 2) || [];
      assert.equal(
        states.length,
        3,
